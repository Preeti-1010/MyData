...
if ( ! function_exists( 'islemag_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function islemag_posted_on() {
...
if ( ! function_exists( 'islemag_entry_footer' ) ) :
	/**
...
function islemag_categorized_blog() {
	...
	if ( $all_the_cool_cats === false ) {
		...
		set_transient( 'islemag_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so islemag_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so islemag_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in islemag_categorized_blog.
 */
function islemag_category_transient_flusher() {
