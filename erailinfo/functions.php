...
function pnr_status($pnrnumber){
	
	$url = 'https://api.railwayapi.com/v2/pnr-status/pnr/'.$pnrnumber.'/apikey/'.apikey.'/'; 
	$jsondata = file_get_contents($url);
	return json_decode($jsondata, true);		
}

function running_status($train_number,$doj){
	
	$url = 'https://api.railwayapi.com/v2/live/train/'.$train_number.'/date/'.$doj.'/apikey/'.apikey.'/'; 
	$jsondata = file_get_contents($url);
	return json_decode($jsondata, true);	
}

function train_between_stations($source_code, $dest_code, $doj){

	$url = 'https://api.railwayapi.com/v2/between/source/'.$source_code.'/dest/'.$dest_code.'/date/'.$doj.'/apikey/'.apikey.'/';
	$jsondata = file_get_contents($url);
	return json_decode($jsondata, true);
	
}

function train_schedule($train_number){

	$url = 'https://api.railwayapi.com/v2/route/train/'.$train_number.'/apikey/'.apikey.'/';
	$jsondata = file_get_contents($url);
	return json_decode($jsondata, true);
		
}

function trains_arrival($station_code,$hours){
		$url = 'https://api.railwayapi.com/v2/arrivals/station/'.$station_code.'/hours/'.$hours.'/apikey/'.apikey.'/';
		$jsondata = file_get_contents($url);
		return json_decode($jsondata, true);
}

function train_information($train_number){

		$url = 'https://api.railwayapi.com/v2/name-number/train/'.$train_number.'/apikey/'.apikey.'/';
		$jsondata = file_get_contents($url);
		return json_decode($jsondata, true);
}

function cancelled($cancelled_date){

		$date = str_replace('_','-',$cancelled_date);
		$url = 'https://api.railwayapi.com/v2/cancelled/date/'.$date.'/apikey/'.apikey.'/';
		$jsondata = file_get_contents($url);
		return json_decode($jsondata, true);
}

function rescheduled_trains($jurney_date){

		$doj = str_replace('_','-',$jurney_date);
		$url = 'https://api.railwayapi.com/v2/rescheduled/date/'.$doj.'/apikey/'.apikey.'/';
		$jsondata = file_get_contents($url);
		return json_decode($jsondata, true);
	}

function all_train($train_number, $postId){

	$url = 'https://api.railwayapi.com/v2/route/train/'.$train_number.'/apikey/'.apikey.'/';
	$jsondata = file_get_contents($url);
	$response = json_decode($jsondata, true);
	if($response['response_code']== 200){
		update_post_meta($postId,'train_info',$jsondata);
		$post = array(
		    'ID' => $postId,
...
function all_station($station_code,$hours, $postId){

	$url = 'https://api.railwayapi.com/v2/arrivals/station/'.$station_code.'/hours/'.$hours.'/apikey/'.apikey.'/';
	$jsondata = file_get_contents($url);
	$response = json_decode($jsondata, true);
	if($response['response_code']== 200){
		update_post_meta($postId,'station_info',$jsondata);
	}
	return $response;
}

function seat_availability($train_number,$source_code,$dest_code,$doj,$seat_class,$quota){
		$doj = str_replace('_','-',$doj);
		$url = 'https://api.railwayapi.com/v2/check-seat/train/'.$train_number.'/source/'.$source_code.'/dest/'.$dest_code.'/date/'.$doj.'/pref/'.$seat_class.'/quota/'.$quota.'/apikey/'.apikey.'/';
		$jsondata = file_get_contents($url);
		return json_decode($jsondata, true);
}

function train_fare_enquiry($train_number,$src,$dest,$age,$quota,$doj,$seat_class)
{
	$doj = str_replace('_','-',$doj);
	$url = 'https://api.railwayapi.com/v2/fare/train/'.$train_number.'/source/'.$src.'/dest/'.$dest.'/age/'.$age.'/pref/'.$seat_class.'/quota/'.$quota.'/date/'.$doj.'/apikey/'.apikey.'/';
	$jsondata = file_get_contents($url);
	return json_decode($jsondata, true);
}

add_filter("gform_field_validation_2_4", "validate_pnr_form", 10, 4);
    function validate_pnr_form($result, $value, $form, $field){
		if (strlen($value) != 10) { //Minimum number of characters
		    $result["is_valid"] = false;
		    $result["message"] = "Enter your 10 digit PNR number.";
		}
		return $result;
		    }

add_filter( 'gform_field_validation', 'validate_all_form', 10, 4);   
	function validate_all_form($result, $value, $form, $field){
		if ( $field['type'] === 'select' && $field['choices'][0]['text'] === $value ) {
		  $is_valid = false;
		}
		else {
		  $is_valid = true;
...
add_filter("gform_pre_render_7", "seat_enquiry");
function seat_enquiry($form){ ?>
     ...
     jQuery('#input_7_8').bind('change', function()
        {
            ...
            var selectedValue = str.pop();
         	jQuery.ajax({
         		url: '<?php echo site_url()?>/trainresult.php',
         		method:'POST',
         		dataType:'json', 
...
add_filter("gform_pre_render_6", "train_enquiry");
function train_enquiry($form){ ?>
    <script type="text/javascript">
    jQuery(document).ready(function(){
jQuery('#input_6_8').bind('change', function()
        {
   
            var arr = jQuery("#input_6_8").val();
            var str = arr.split('-');
            //console.log(str);
            var choosenvalue = str.pop(); 
         	jQuery.ajax({
         		url: '<?php echo site_url()?>/trainresult.php',
         		method:'POST',
         		dataType:'json', 
         		data:{trainid:choosenvalue},
         		success:function(response){
					jQuery("#input_6_2").removeAttr('style');
					jQuery("#input_6_2").empty();
					jQuery("#input_6_6").empty();
         			jQuery.each(response, function(index, data){    
         				console.log(data);
         			var post_title = data.post_title;
         			var stnA = post_title.split('-');
         			var stnC = stnA.pop();
         			jQuery("#input_6_2").append('<option value="'+ stnC+'">'+data.post_title+'</option>'); 				
         			var train_data="<option value="+stnC+">"+data.post_title+"</option>";
         			jQuery(train_data).appendTo('#input_6_6');
         			});
...
add_action('init', function() {
  ...
  //Live Status//
 	if (strpos($url_path, 'live-train-status/') !== false) {
     // load the file if exists
     $load = locate_template('template-live.php', true);
     if ($load) {
    ...
    //Between Station//
 	if (strpos($url_path, 'trains-between-stations/') !== false) {
     // load the file if exists
     $load = locate_template('template-btwstation.php', true);
     if ($load) {
        exit(); // just exit if template was found and loaded
     }
  }
   ...
   //Train Arrival//
 	if (strpos($url_path, 'trains-arrival-at-stations/') !== false) {
     // load the file if exists
     $load = locate_template('template-arrival.php', true);
     if ($load) {
        exit(); // just exit if template was found and loaded
     }
  }
  ...
  //Train Schedule//
 	if (strpos($url_path, 'irctc-trains-schedule/') !== false) {
     // load the file if exists
     $load = locate_template('template-schedule.php', true);
     if ($load) {
        exit(); // just exit if template was found and loaded
     }
  }
  ...
  //Train Enquiry//
 	if (strpos($url_path, 'train-fare-enquiry/') !== false) {
     // load the file if exists
     $load = locate_template('template-train-fare-enquiry.php', true);
     if ($load) {
        exit(); // just exit if template was found and loaded
     }
  }
   ...
   //Train Availability//
 	if (strpos($url_path, 'seat-availability/') !== false) {
     // load the file if exists/cancelled-trains/
     $load = locate_template('template-seat-availabilty.php', true);
     if ($load) {
        exit(); // just exit if template was found and loaded
     }
  }
   ...
   //Cancelled Train//
 	if (strpos($url_path, 'cancelled-trains/') !== false) {
     // load the file if exists
     $load = locate_template('template-cancelledtrain.php', true);
     if ($load) {
        exit(); // just exit if template was found and loaded
     }
  }
   ...
   //Resheduled Train//
 	if (strpos($url_path, 'rescheduled-trains/') !== false) {
     // load the file if exists 	
     $load = locate_template('template-reschedule.php', true);
     if ($load) {
        exit(); // just exit if template was found and loaded
     }
  }
  ...
  //Train Information//
 	if (strpos($url_path, 'indian-train-information/') !== false) {
     // load the file if exists 	
     $load = locate_template('template-traininfo.php', true);
     if ($load) {
        exit(); // just exit if template was found and loaded
     }
  }
  ...
  //Pnr Status//
 	if (strpos($url_path, '/live-train-pnr-status/') !== false) {
     // load the file if exists 	
     $load = locate_template('template-pnr.php', true);
     if ($load) {
        exit(); // just exit if template was found and loaded
     }
  }
...
add_action( 'wp_ajax_nopriv_post_stations', 'post_stations' );
add_action( 'wp_ajax_post_stations', 'post_stations' );

function post_stations() {
	$args = array(
	    'post_type' => 'station',
	    's' => $_GET['q']['term'],
	    'post_status' => 'publish',
	...
	if ($wp_query->have_posts()) {
		while ($wp_query->have_posts()): $wp_query->the_post();
			//$result[$wp_query->post->post_name]=get_the_title();
			$result[] = ['id'=>$wp_query->post->post_name, 'text'=>get_the_title()];
		endwhile;
...
add_action( 'wp_ajax_nopriv_post_trains', 'post_trains' );
add_action( 'wp_ajax_post_trains', 'post_trains' );

function post_trains() {
	$args = array(
	    'post_type' => 'train',
	    's' => $_GET['q']['term'],
	    'post_status' => 'publish',
