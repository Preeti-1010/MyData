...
function islemag_refresh_general_control_values(){
	...
	jQuery( '.islemag_general_control_repeater' ).each(
		function(){
			var values = [];
			var th     = jQuery( this );
			th.find( '.islemag_general_control_repeater_container' ).each(
				function(){
...
jQuery( document ).ready(
	function(){
		...
		/* Dropdown control */
		theme_conrols.on(
			'click','.islemag-customize-control-title',function(){
		...
		jQuery( '.islemag_general_control_new_field' ).on(
			'click',function(){
		...
		theme_conrols.on(
			'click', '.islemag_general_control_remove_field',function(){
		...
		theme_conrols.on(
			'keyup', '.islemag_link_control',function(){
				islemag_refresh_general_control_values();
			}
		);

		jQuery( '.icp' ).iconpicker().on(
			'iconpickerUpdated', function() {
				islemag_refresh_general_control_values();
			}
		);
